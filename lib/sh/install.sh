...
if command -v curl >/dev/null 2>&1; then
  mkdir -p /tmp/zi
  cd /tmp/zi || return
  curl -fsSLO https://raw.githubusercontent.com/z-shell/zi/main/lib/zsh/git-process-output.zsh &&
    chmod a+x /tmp/zi/git-process-output.zsh
elif command -v wget >/dev/null 2>&1; then
  mkdir -p /tmp/zi
  cd /tmp/zi || return
  wget -q https://raw.githubusercontent.com/z-shell/zi/main/lib/zsh/git-process-output.zsh &&
    chmod a+x /tmp/zi/git-process-output.zsh
fi
...
RCUPDATE=1
if grep -E '(zi|zinit)\.zsh' "${THE_ZDOTDIR}/.zshrc" >/dev/null 2>&1; then
  echo "[34m▓▒░[0m .zshrc already contains \`zi …' commands – not making changes."
  RCUPDATE=0
fi
...
if [[ ! -f ${ZI_HOME}/${ZI_BIN_DIR_NAME}/zi.zsh ]]; then
  ...
  command git clone -q https://github.com/z-shell/zi "${ZI_HOME}/${ZI_BIN_DIR_NAME}" && \\
...
EOF
  echo
  echo "[38;5;219m▓▒░[0m Would you like to add annexes to the zshrc?" \
